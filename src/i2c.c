...
/* This library is generic wrt peripheral device, except for the timing params
 */
...
#define SDA_PIN RA1          // Pin to read I2C input from when receiving
#define SCL_TCLK_US_DIV_3 20 // 1/3 of I2C bus clock period in microseconds
...
static bool byte_out(uint8_t dout) {
    ...
    for (uint8_t i = 0; i < 8; i++) {
        ...
        __delay_us(SCL_TCLK_US_DIV_3); // setup time
        SCL = 1;                       // clock rise
        __delay_us(SCL_TCLK_US_DIV_3); // clock high
        SCL = 0;                       // clock fall
        __delay_us(SCL_TCLK_US_DIV_3); // hold time
    }
    ...
    SDA = 1;
    __delay_us(SCL_TCLK_US_DIV_3);   // setup time (for device)
    SCL = 1;                         // clock rise
    __delay_us(SCL_TCLK_US_DIV_3/2);
    bool ack = SDA_PIN;              // Sample pin halfway through clock pulse
    __delay_us(SCL_TCLK_US_DIV_3/2);
    SCL = 0;                         // clock fall
    __delay_us(SCL_TCLK_US_DIV_3);   // hold time
 ...
 * Inputs:
 *   trgt_addr: I2C bus address of target peripheral
 *   direction: transfer dir, I2C_RD or I2C_WR
 *   ctrl_data: pointer to uint8_t array in controller to being transfer at
...
uint8_t i2c_transfer_bytes(uint8_t tgrt_addr, bool direction, uint8_t* ctrl_data, uint8_t prph_addr, uint8_t num_bytes) {
    ...
    SDA = 0;
    __delay_us(SCL_TCLK_US_DIV_3); // t_HD:STA, must be > 4us, use Tclk/3
    SCL = 0;
    ...
    if (byte_out((tgrt_addr << 1) | I2C_WR)) {
        return 1;
    }
    ...
    if (direction == I2C_RD) { // if read
        ...
        __delay_us(SCL_TCLK_US_DIV_3); // buffer
        SDA = 1;
        __delay_us(SCL_TCLK_US_DIV_3); // t_SU:DAT (even though not transmitting data)
        SCL = 1;
        __delay_us(SCL_TCLK_US_DIV_3); // t_SU:STA > 4.7us
        SDA = 0;
        __delay_us(SCL_TCLK_US_DIV_3); // t_HD:STA > 4.0us
        SCL = 0;

        // Transmit I2C address again, this time w R/!W = 1
        if (byte_out((tgrt_addr << 1) | I2C_RD)) {
            return 3;
        }
        ...
        for (uint8_t i = 0; i < num_bytes-1; i++) {
            ctrl_data[i] = byte_in(0);
        }
        ctrl_data[num_bytes-1] = byte_in(1);
    }
    else { // write
    
        for (uint8_t i = 0; i < num_bytes; i++) {
            if (byte_out(ctrl_data[i])) {return i+4;}
        }
    }
    ...
    __delay_us(SCL_TCLK_US_DIV_3); // buffer
    SCL = 1;
    __delay_us(SCL_TCLK_US_DIV_3); // t_SU:STO > 4.7us
    SDA = 1;
    __delay_us(SCL_TCLK_US_DIV_3); // buffer
...
uint8_t i2c_sync_intf(uint8_t num_clocks) {
    ...
    // Then the only way for SDA to be read as zero is if a periph is pulling
    // it low bc it's desynced or in some weird state. 
    SCL = 1;
    SDA = 1;
    
    // Toggle SCL a bunch and then check if the periph has released SDA (if it
    ...
    // If SDA is still not high (periph pulling it down), sync failed
    if (!SDA_PIN) {return 2;}
